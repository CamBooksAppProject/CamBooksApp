...
export default function ChatScreen() {
    ...
    const renderChatItem = (item) => (
        <TouchableOpacity
            ...
            key={item.id}
            onPress={() => navigation.navigate("ChatDetailPage", { chat: item })} // 채팅 항목 클릭 시 ChatPage로 이동
        >
            <Image
                source={IMAGES.LOGOV2} // 프로필 사진  source={item.profileImage} // 각 사용자의 프로필 이미지 하려면 변경하면됨.
                style={styles.myImg}
            />
